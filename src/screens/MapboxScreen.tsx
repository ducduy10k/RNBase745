  ...
  return (
    <View style={styles.page}>
      <Mapbox.MapView
        ...
        {
          lstMarker.map(item => {
            return <Mapbox.MarkerView key={item.id} id={item.id} coordinate={item.coordinates} >
          <CustomIcon name={item.iconName} lib={item.lib} size={item.size} color={item.color}/>
        </Mapbox.MarkerView>
          })
        }
      ...
      <View style={styles.btnControlTopRight}>
        {cameraRef.current && <SearchLocationControl mapCamera={cameraRef.current} onSearch={(item) => handleSearch(item)}/>}
